...
- **Regenerated `llms.txt` / `llms-full.txt`** to include the runnable-demo README section (they predated it). This is the exact drift issue #27 exists to gate — the committed machine-readable corpus fell behind a docs change with nothing to catch it. Until #27 lands, regenerate with `python3 scripts/gen-llms.py` after any docs change.

- **The root README now opens with something you can run.** `make demo && ./build/door` prints `state=1 events=1 hash=…` in about thirty seconds — one bounded state cell, one authorized action, one committed transition, one stable hash. It was documented only at line 60 of `app/README.md`, so a first-time reader met several hundred lines of ontology before encountering any evidence the substrate executes. The hash is the part that matters: the same declaration produces the same number on any conforming build, under GCC or Clang, which is the corpus's central claim reduced to one checkable line.

- **Added `llms.txt` and `llms-full.txt`** (`scripts/gen-llms.py`), so the corpus is consumable by language models as a single artifact. The generator is built on an **allowlist**, not a filter: `LICENSES.md` states that third-party and excluded material must not be treated as project-licensed even when co-located, and that an unlisted path is *unlicensed pending classification*. Concatenating `docs/**` wholesale would have inlined third-party literature and bibliographic metadata into one file carrying the project's CC BY 4.0 banner. External literature, citation metadata, and archived releases are therefore **named in the index and never inlined** — 31 documents excluded, each with a stated reason. The full text opens with the program's own status caveat so a model ingesting it inherits the honesty framing rather than the claims alone.
